...
//Connection Settings @0-5C64AF13
...
if (!function_exists("mb_strpos")) {
    echo "This application requires PHP mb extension to handle international encodings.<br>You need to enable mb extension in PHP.INI (Windows) or recompile PHP (*nix)<br><br>Refer to <a href='http://www.php.net/manual/en/ref.mbstring.php'>PHP documentation</a>";
    exit;
}


//Initialize Common Variables @0-B49EEC06
...
$CCSLocales->AddLocale("pl", Array("pl", "PL", array(1, 0, ""), 2, ",", " ", array("styczeń", "luty", "marzec", "kwiecień", "maj", "czerwiec", "lipiec", "sierpień", "wrzesień", "październik", "listopad", "grudzień"), array("sty", "lut", "mar", "kwi", "maj", "cze", "lip", "sie", "wrz", "paź", "lis", "gru"), array("niedziela", "poniedziałek", "wtorek", "środa", "czwartek", "piątek", "sobota"), array("N", "Pn", "Wt", "Śr", "Cz", "Pt", "So"), array("N", "P", "W", "Ś", "C", "P", "S"), array("yyyy", "-", "mm", "-", "dd"), array("d", " ", "mmmm", " ", "yyyy"), array("HH", ":", "nn"), array("HH", ":", "nn", ":", "ss"), "", "", 1, false, "", "utf-8", "UTF-8"));
...
//IM Connection Class @-A77DB80E
class clsDBIM extends DB_MySQL
{
    ...
    function Initialize()
    {
        ...
        $Configuration = $CCConnectionSettings["IM"];
...
function CCSubStr($str, $offset, $length = null, $encoding = false) {
 global $FileEncoding;
 if (false === $encoding) $encoding = $FileEncoding;
 if (null === $length) $length = CCStrLen($str, $encoding) - $offset;
...
//CCLoginUser @0-6ED1808C
function CCLoginUser($login, $password)
{
    $db = new clsDBIM();
    $SQL = "SELECT user_id, security_level FROM users WHERE login=" . $db->ToSQL($login, ccsText) . " AND pass=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    ...
    if($Result)
    {
        ...
        CCSetSession("GroupID", $db->f("security_level"));
    }
    $db->close();
    return $Result;
}
...
{
	$method = GetSetting("email_component");
	switch ($method)
	{
		case 0:
			break;
		case 1:
			echo "<b>From:</b> $mailFrom<br><b>To:</b> $mailTo<br><b>Subject:</b> $mailSubject<br><pre>$mailBody</pre><hr>";
			break;
		case 8:
...
{
	return CCDLookup($variable,"settings","settings_id=1",new clsDBIM());
}

function SendNotification($message, $user_id, $issue_id)
{
	$db = new clsDBIM();
	$curUserName = CCDLookup("user_name","users","user_id=" . CCGetSession("UserID"),$db);
	$receiver = CCDLookup("user_name","users","user_id=$user_id", $db);
	$issue_name = CCDLookup("issue_name","issues","issue_id=$issue_id", $db);
	$issue_desc = CCDLookup("issue_desc","issues","issue_id=$issue_id", $db);

	$mailBody = GetSetting($message . "_body");
	$mailBody = str_replace("\r\n","<br>",$mailBody);
	$mailBody = str_replace("{issue_no}",$issue_id,$mailBody);
	$mailBody = str_replace("{issue_title}",$issue_name,$mailBody);
	$mailBody = str_replace("{issue_desc}",$issue_desc,$mailBody);
	$mailBody = str_replace("{issue_url}","<a href='http://" . $_SERVER["SERVER_NAME"] . ereg_replace("/[^/]+$", "/IssueChange.php?issue_id=$issue_id",$_SERVER["SCRIPT_NAME"]) . "'>here</a>",$mailBody);
	$mailBody = str_replace("{issue_poster}",$curUserName,$mailBody);
	$mailBody = str_replace("{issue_receiver}",$receiver,$mailBody);

	$mailSubject = GetSetting($message . "_subject");
	$mailSubject = str_replace("{issue_no}",$issue_id,$mailSubject);
	$mailSubject = str_replace("{issue_title}",$issue_name,$mailSubject);
	$mailSubject = str_replace("{issue_poster}",$curUserName,$mailSubject);
	$mailSubject = str_replace("{issue_receiver}",$receiver,$mailSubject);

	SendEMail(GetSetting($message . "_from"),CCDLookup("email","users","user_id=$user_id", $db),$mailSubject,$mailBody);
}

function SetupUpload(&$control)
{
  $control->AllowedFileMasks = GetSetting("file_extensions");
  $control->FileFolder = GetSetting("file_path") . "/";
}

function PopulateFiles(&$link)
{		
		global $CCSLocales;
		$db = new clsDBIM();
		$list = "";
		$sql = "SELECT file_name FROM files WHERE issue_id=0 AND uploaded_by=" . CCGetSession("UserID");
		$db->query($sql);
		$filesFound = false;
		$folder = GetSetting("file_path");
		while($db->next_record())
		{
			$filename = $db->f("file_name");
			$list .= "<a target='_blank' href='$folder/$filename'>" . CCGetOriginalFilename($filename) . "</a><br>";
			$filesFound = true;
		}
		$link = $filesFound?$CCSLocales->GetText("im_edit_files"):$CCSLocales->GetText("im_add_files");
		return $list;
}
...
{
		$db = new clsDBIM();
		$db->query("DELETE FROM files WHERE issue_id=0 AND uploaded_by=".$db->ToSQL(CCGetUserID(),ccsInteger));
}

function Translate($str)
{
	global $CCSLocales;
	if (CCSubStr($str,0,4) === "res:") $str = $CCSLocales->GetText(CCSubStr($str,4));
	return $str;
}

function TranslateBoolean(&$form)
{
	$fields = array("tested", "approved", "tested1", "approved1");
	foreach ($fields as $field)
		if (@$form->$field)
			foreach ($form->$field->Format as $i=>$item)
				$form->$field->Format[$i] = Translate($form->$field->Format[$i]);
}

function FormatBoolean(&$control, $yes="res:im_yes", $no="res:im_no")
{
	$format = array(Translate($yes), Translate($no), "");
	if (@$control)
		$control->Format = $format;
}

function TranslateListbox(&$listbox)
{
	for ($i = 0; $i < sizeof($listbox->Values); $i++)
		$listbox->Values[$i][1] = Translate($listbox->Values[$i][1]);
}

function GetGeneralDateFormat()
{
	global $CCSLocales;
	return join("",$CCSLocales->GetFormatInfo("GeneralDate"));
}
